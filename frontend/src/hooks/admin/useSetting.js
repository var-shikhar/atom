...
const useSettingPanel = () => {
    ...
    const settingList = [
        ...
        },
        {
            ...
            tableData: filteredSubCategoryList,
        },
    ]
    ...
    useEffect(() => {
        if(subCategoryList?.length > 0){
            let tempSubCategory = subCategoryList;
            if(activeCategory !== ''){
                tempSubCategory = tempSubCategory.filter(item => item?.categoryId?._id?.toString() === activeCategory?.toString())
            }

            startTransition(() => {
                setFilteredSubCategoryList(tempSubCategory);
            })
        }
    }, [subCategoryList, activeCategory])
