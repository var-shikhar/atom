...
const SettingPanel = () => {
  const { handleDeleteItem, handleRefreshTkn, handleItemStatus, settingList, setActiveCategory } = useSettingPanel();
              ...
              <Card.Header as="h5" className='bg-body-background text-light'>
                <div className='d-flex align-items-center justify-content-between'>
                  ...
                  <div className='d-flex gap-2 align-items-center'>
                    {activeNavIndex === 1 && 
                      <select onChange={(e) => setActiveCategory(e.target.value)} className='form-control'>
                        <option value={''}>Filter by Category</option>
                        {settingList[0].tableData?.map(item => <option value={item._id} key={item._id}>{item.name}</option>)}
                      </select>
                    }
                    <Button type='button' className='max-content' onClick={() => handleForm({
                      id: '', 
                    ...
                    {settingList[activeNavIndex].tableData?.map((item, index) => {
                      ...
                      return (
                        <tr key={item._id}>
                          ...
                          {activeNavIndex === 1 && <>
                            <td colSpan={1}>{item?.categoryId?.name}</td> 
                            <td colSpan={1}>{item?.categoryTax}%</td> 
                          </>}
