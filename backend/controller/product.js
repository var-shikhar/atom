...
// Get Public List
const getPublicProductList = async (req, res) => {
    try {
        const foundProducts = await Product.find({ isAvailable: true }).populate('categoryId').sort({ launchDate: -1 });
        let productList = [];

        if(foundProducts?.length > 0){
            productList = await Promise.all(foundProducts?.map(async product => {
                return {
                    id: product._id,
                    ...
                    hasVariation: product.isVariationProduct,
...
const getLatestProduct = async (req, res) => {
    try {
        ...
        const foundProduct = {
            ...
            category: latestProduct.categoryId.name,
...
export default {
    getProductList, getPublicProductList, getPublicProductDetails, getLatestProduct,
